...
sequence headers using different conventions. Understanding these
formats is essential when downloading databases, running taxonomic
classifiers, and summarizing results with dbpq.
...
Used by: **UNITE** (SINTAX release), **PR2** (UTAX format), **MIDORI2**
(SINTAX format), **Eukaryome** (SINTAX format)
...
This format is used by `vsearch --sintax` and the original USEARCH UTAX
algorithm. Note that SINTAX and UTAX use the same header format despite
being different classification algorithms.
...
Similar to the UNITE format but starts with `d__` (domain) instead of
`k__` (kingdom):
...
Some databases use semicolon-separated taxonomy without any prefix. The
meaning of each rank is determined by its position in the string. The
number of levels varies by database.

For these formats, use `rank_position` in
[`list_ranks_db()`](https://adrientaudiere.github.io/dbpq/reference/list_ranks_db.md)
to extract a specific rank by position.
...
classifier accepts any number of semicolon-separated levels — with or
without prefixes — via the `taxLevels` argument.
...
format used by SILVA and RDP training sets, there is typically no
separate accession number — the header contains only taxonomy, and the
deepest level often serves as a sequence identifier.
...
must correctly extract or relocate the sequence name.
...
returns rank prefixes (character vector) for prefix-based formats and
